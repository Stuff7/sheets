...
function template(html) {
  ...
  let cached;
  return () => (cached || (cached = createNode())).cloneNode(true);
}
    ...
    mutations.forEach((mutation) => {
      ...
      for (const node of mutation.addedNodes) {
        queueMicrotask(() => {
          iterChildrenDeep(
            node,
            (node2) => node2.dispatchEvent(new CustomEvent("mount"))
          );
        });
      }
      for (const node of mutation.removedNodes) {
        queueMicrotask(() => {
          iterChildrenDeep(
            node,
            (node2) => node2.dispatchEvent(new CustomEvent("unmount"))
          );
        });
      }
    });
